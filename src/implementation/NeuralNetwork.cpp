...
void NeuralNetwork::shutdown() {
	visualiserInterface->joinAllThreads();
}
#pragma endregion
...
void NeuralNetwork::updateParameters() {
	auto u = visualiserInterface->getParametersFromVisualiser();
	activationType = std::get<0>(u);
	learningRate = std::get<1>(u);
	maxEpochs = std::get<2>(u);
}
#pragma endregion
...
void NeuralNetwork::test() {
	...
	visualiserInterface->updateAccuracyStatistic(accuracy);
}
...
void NeuralNetwork::accumulateGradients(std::vector<float>& targetVals, Gradients& grads) {
	std::vector<std::vector<float>> deltas(layers.size());
...
void Gradients::apply(std::vector<std::vector<std::vector<float>>>& weights,
	std::vector<Layer>& layers,
